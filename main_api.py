...
async def ep_ping():
    d = {
        ...
        "boat_name": ddh_config_get_vessel_name(),
...
async def ep_monitored_macs():
    d = {
        "monitored_macs": ddh_config_get_monitored_pairs(),
    }
    return d
...
async def api_get_info():
    ...
    d = {
        ...
        "boat_prj": _th(ddh_config_get_box_project),
        "boat_sn": _th(ddh_config_get_box_sn),
        "boat_name": _th(ddh_config_get_vessel_name),
        "has_lowell_loggers": _th(ddh_config_contains_monitored_lowell_loggers),
        "running": _th(api_get_running_ddh),
...
async def ep_logs_get_with_since(start, end):
    ...
    start = str(ts_start.year) + '-' + str(ts_start.month).zfill(2) + '-' + \
            str(ts_start.day).zfill(2)
    end = str(ts_end.year) + '-' + str(ts_end.month).zfill(2) + '-' + \
            str(ts_end.day).zfill(2)
...
@app.get("/dws_start")
async def ep_api_dws_start():
    rv = api_dws_start()
    if rv.returncode:
        return {'dws_start': CTT_API_ER}
